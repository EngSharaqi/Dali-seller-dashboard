			...
			<div class="card-body">
					...
					<li class="list-group-item">
						<div class="row align-items-center">
							<div class="col-6">
								<a href="{{ url('/seller/product/'.$row->term->id.'/edit') }}">{{ $row->term->title ?? '' }}<br>
									...
									@endphp
                                @if(isset($variations->attribute)||isset($variations->options))
                                    @if (is_array($variations->attribute)) {
                                    ...
                                    @endforeach
                                    @endif
                                    @if (is_array($variations->options)) {
						...
						<li class="list-group-item">
							<div class="row align-items-center">
								...
								<div class="col-3 text-right">@isset($order_content->coupon_discount){{ amount_format($order_content->coupon_discount) }} @else no discount found @endif</div>
							</div>
						</li>
						<li class="list-group-item">
							<div class="row align-items-center">
								<div class="col-9 text-right">{{ __('Subtotal') }}</div>
								<div class="col-3 text-right">@isset($order_content->sub_total) {{ amount_format($order_content->sub_total) }} @else 0 @endif</div>
							</div>
						</li>
						<li class="list-group-item">
							<div class="row align-items-center">
								<div class="col-9 text-right">{{ __('Total') }}</div>
								<div class="col-3 text-right">@isset($order_content->total){{ amount_format($info->total) }} @else 0 @endif</div>
							</div>
						</li>
					</ul>
				</div>
				...
				<div class="card-footer">
					...
					<div class="text-right">
						...
						<form method="POST" action="{{ route('seller.orders.update',$info->id) }}" accept-charset="UTF-8" class="d-inline basicform">
							@csrf
		...
		<div class="col-12 col-lg-4">
			<div class="card-grouping">
				...
				<div class="card">
					...
					<div class="card-body">
						<p class="mb-0">@isset($order_content->comment){{ $order_content->comment }} @else no comments found @endif</p>
					</div>
				</div>
				<div class="card">
					<div class="card-header">
